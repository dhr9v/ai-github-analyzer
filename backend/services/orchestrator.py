...
class AnalysisOrchestrator:
    ...
    @classmethod
    def run_analysis(cls, analysis_id: int, db):
        """
        Runs the complete analysis pipeline sequentially:
        Cloning -> Metadata detection -> Static Analysis -> AST Complexity -> Gemini Review -> Save results.
        """
        from sqlalchemy.orm import Session
        from backend.models.models import Analysis, Issue, Repository, Setting

        analysis = db.query(Analysis).filter(Analysis.id == analysis_id).first()
        if not analysis:
            logger.error(f"Analysis record {analysis_id} not found in DB.")
            return
        
        repo = db.query(Repository).filter(Repository.id == analysis.repo_id).first()
        if not repo:
            analysis.status = "failed"
            analysis.error_message = "Repository record not found."
            db.commit()
            return
            
        user_settings = db.query(Setting).filter(Setting.user_id == repo.user_id).first()
        github_pat = user_settings.github_pat if user_settings else None
        gemini_model = user_settings.gemini_model if user_settings else "gemini-2.5-flash"
        
        repo_dest = os.path.join(settings.CLONE_DIR, f"repo_{repo.id}_analysis_{analysis.id}")
        
        try:
            # 1. Cloning
            logger.info(f"Starting cloning for Analysis {analysis_id}")
            ...
            repo.current_commit = commit_hash
            db.commit()
            
            # 2. Local Static & Complexity Analysis
            logger.info(f"Starting static checks for Analysis {analysis_id}")
            ...
            # Save static issues to DB
            for issue_data in static_issues:
                issue = Issue(
            ...
            # 3. Gemini Context Preparation & Invocation
            logger.info(f"Requesting Gemini AI Review for Analysis {analysis_id}")
            ...
            # Setup reviewer with the API key from config or settings override
            ...
            # Populate scores in Analysis
            analysis.overall_score = review_res.overall_score
            analysis.security_score = review_res.security_score
            analysis.performance_score = review_res.performance_score
            analysis.maintainability_score = review_res.maintainability_score
            analysis.documentation_score = review_res.documentation_score
            analysis.testing_score = review_res.testing_score
            analysis.architecture_score = review_res.architecture_score
            
            analysis.executive_summary = review_res.executive_summary
            analysis.strengths = review_res.strengths
            analysis.weaknesses = review_res.weaknesses
            analysis.complexity_data = complexity_results
            analysis.refactoring_suggestions = review_res.refactoring_suggestions
            
            # Save Gemini audit issues
            for issue_data in review_res.issues:
                # Sanitize file paths
            ...
            logger.error(f"Failed analysis run for ID {analysis_id}: {str(e)}", exc_info=True)
            analysis.status = "failed"
            analysis.error_message = str(e)
            db.commit()
        finally:
            # Assure cleanup of the cloned files
            ClonerService.cleanup_dir(repo_dest)
